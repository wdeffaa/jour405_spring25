---
title: "HW11_Education_Level_and_Survey_Weighting"
...
Education level samples may differ from the general population because some may not want to possibly want to reveal their eduction levels. Some factors that may cause certain education groups to be less represented than others would possibly be large groupings such a "High School or Less," which may possibly make the amount of people with a high school education level less. 
...
People with some college or college degrees have a higher intention to vote.  
...
There seemed to be very little change in percentages through most of the table. The biggest change occured in the "High School or less" category in which the percentage slightly dropped for "Yes" and there was a slight increase in "No" and "Unsure" columns. 

2. Why might the weighted results be considered more accurate than the unweighted results?

Because weighted results even out the sample size for each different category being surveyed, not counting one vote as more than a single vote.
...
"High School or less" shows the largest difference in results between weighted and unweighted.
...
I think it does do a good job visualizing the differences between the weighted and unweighted results, however, I personally feel like there is not much of a need for visualization because the only noticeable change is in the "High School and less" category.
...
Education level relates quite a bit to voting intentions as people who have higher levels of education than High School seem more likely to intend on voting in the upcoming election. Weighting slightly affected my understanding of the relationship between education level and voting by showing that results that are unweighted are overinflated than results are weighted and therefore weighted results seem to be more accurate. This means that when interpreting polling we should consider many factors that went into polling, including how every vote is weighted, as even the slight difference between weighted and unweighted results can be large.   
...
It is important to understand weighting when reporting on polls so you can fully understand how each vote may be weighed and to better intrepret polling as a whole.

2. How might the differences between weighted and unweighted results affect how you would report on this data?

I think that the differences in weighted and unweighted data would impact my reporting on this data by taking into consideration the how even the little changes between the two could have a big impact in polling nationwide.
...
Additional information I would want to know regarding how the weights are calculated would be were there other variables used to calculate these weights, was a certain group of people polled for this or was the variety in people polled outside of education level still high?
