...
The variables that are the most positively correlated are income and the percent that have a bachelors or higher, and turnout and percentage that are white. The variables that are the most negatively correlated are percentage that are unemployed and turnout. The only variables that seem to have no correlation with one another are percentage with a bachelors or higher and the median age. In terms of the scatterplot, the variables that seem to have a linear relationship are income and the percentage that are unemployed.    
...
This model predict turns out well and I believe is a useful model because the Min. and Max. are not super far-reached, and the median is less than 1. Some variables that seem insignificant are population, the percentage with a bachelors or higher, and the percentage that are unemployed. 
...
For my test, model 2 seems to be significantly better than model 3 and 4.

```{r}
# Model 2
model2 <- lm(turnout ~ pct_unemployed, data = md_turnout)
summary(model2)
...
# Model 4
model4 <- lm(turnout ~ population, data = md_turnout)
summary(model4)
...
I would use this model to highlight some theories that people have about voter participation in elections and how education level really isn't a major factor in voter turnout, while most people who are unemployed often do not vote in elections.   
