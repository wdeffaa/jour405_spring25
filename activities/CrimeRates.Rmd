...
```{r}
crimedata <- read_csv("https://raw.githubusercontent.com/dwillis/jour405/refs/heads/main/data/md_annual_crime.csv") %>% clean_names()
```
...
```{r}
crimedata %>% mutate(total_rate = grand_total/population * 10000)
```
...
```{r}
crimes_with_rates <- crimedata %>% mutate(total_rate =property_crime_total/population * 10000, violent_crime_total/population * 10000)
```
...
The late 90s and 80s had the highest amount of crime and since then it has declined to the lowest in 2020. this does not shock me all too much because as we have evolved as a society, crime has continued to go down as well.
